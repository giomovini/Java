...
import teste.aplicacao.LinhaCartaoAcesso;
import teste.aplicacao.LinhaReserva;
...
public class ControllerTelaInicial {
	...
	@FXML
	TableView<LinhaReserva> TabReservas;

	@FXML
	TableColumn<LinhaReserva, String> Locatario_Reserva, Exemplar_Reserva, Data_Reserva;
	
	@FXML
	TableView<LinhaCartaoAcesso> TabCartaoAcesso;

	@FXML
	TableColumn<LinhaCartaoAcesso, String> Locatario_CA, Cod_barras_CA, validade_CA;
	...
	Button InserirLocatario, DeletarLocatario, EditarLocatario, InserirExemplar, EditarExemplar, DeletarExemplar,
			InserirLocacao, DevolverLocacao, Imprimir,RenovarLocacao,InserirReserva,ExcluirReserva,ExcluirCartaoAcesso,InserirCartaoAcesso;
	...
	public static Locacao Locacao_;
	...
	public void initialize() {
		...
		propriedadesCartaoAcesso();

	}
	
	public void propriedadesCartaoAcesso() {
		
		Locatario_CA.setCellValueFactory(new PropertyValueFactory<LinhaCartaoAcesso, String>("locatario"));
		Cod_barras_CA.setCellValueFactory(new PropertyValueFactory<LinhaCartaoAcesso, String>("Cod_Barras"));
		validade_CA.setCellValueFactory(new PropertyValueFactory<LinhaCartaoAcesso, String>("Validade"));
		
		ExcluirCartaoAcesso.setOnAction(e->{
			if (TabCartaoAcesso.getSelectionModel().getSelectedItem() != null) {
				CartaoAcesso Cart = TabCartaoAcesso.getSelectionModel().getSelectedItem().getCartao();
				Cart.excluirCartao(Cart);
				atualiza = true;
			} else {
		...
		InserirCartaoAcesso.setOnAction(e->{
			Tela.getInstance().AbrirTelaSobreposta(Arquivos.inserirCartaoAcesso, Titulos.inserirCartaoAcesso);
		});


		converteListaCartaoAcesso();
	}
	
	public void propriedadesReserva() {

		Locatario_Reserva.setCellValueFactory(new PropertyValueFactory<LinhaReserva, String>("locatario"));
		Exemplar_Reserva.setCellValueFactory(new PropertyValueFactory<LinhaReserva, String>("titulo"));
		Data_Reserva.setCellValueFactory(new PropertyValueFactory<LinhaReserva, String>("DataReserva"));
		
		ExcluirReserva.setOnAction(e->{
			if (TabReservas.getSelectionModel().getSelectedItem() != null) {
				Reserva Res = TabReservas.getSelectionModel().getSelectedItem().getM();
				Res.excluirReserva(Res);
				atualiza = true;
			} else {
				JOptionPane.showMessageDialog(null, "Selecione um item da tabela!");
			}
		...
		InserirReserva.setOnAction(e->{
			Tela.getInstance().AbrirTelaSobreposta(Arquivos.inserirReserva, Titulos.inserirReserva);
		});


		converteListaReserva();
	}
	...
	public void propriedadesLocacao() {
		...
		RenovarLocacao.setOnAction(e -> {
			if (TabLocacao.getSelectionModel().getSelectedItem() != null) {
				Locacao_ = TabLocacao.getSelectionModel().getSelectedItem().getLoc();
				Tela.getInstance().AbrirTelaSobreposta(Arquivos.renovacaoLocacao, Titulos.renovacaoLocacao);
	...
	public void propriedadesLocatario() {
		...
		ImprimirCart.setOnAction(e->{
			if (TabLocatario.getSelectionModel().getSelectedItem() != null) {
				Loc = TabLocatario.getSelectionModel().getSelectedItem().getLoc();
				ImpressaoCarteirinha imprimir = new ImpressaoCarteirinha();
				Loc.setImpressaoCarteirinha(imprimir);
				imprimir.imprimirCarteirinha(Loc);
			} else {
				JOptionPane.showMessageDialog(null, "Selecione um item da tabela!");
			}
	...
	public void converteListaReserva() {
		ObservableList<LinhaReserva> listaReserva = FXCollections.observableArrayList();

		ArrayList<Reserva> ListaReservas = (ArrayList<Reserva>) new Reserva().listarReservas();
		SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");

		for (Reserva res : ListaReservas) {
			LinhaReserva linha = new LinhaReserva();

			linha.setM(res);
			linha.setTitulo(res.getEx().getTitulo());
			linha.setLocatario(res.getLoc().getNome());
			
			linha.setDataReserva(formato.format(res.getDataReserva()));

			listaReserva.add(linha);

		}
		ControllerTelaInicial.atualiza = true;
		TabReservas.setItems(listaReserva);
	}
	
	public void converteListaCartaoAcesso() {
		ObservableList<LinhaCartaoAcesso> listaCartaoAcesso = FXCollections.observableArrayList();

		ArrayList<CartaoAcesso> ListaCartao = (ArrayList<CartaoAcesso>) new CartaoAcesso().listarCartaoAcesso();
		SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");

		for (CartaoAcesso cart : ListaCartao) {
			LinhaCartaoAcesso linha = new LinhaCartaoAcesso();

			linha.setCartao(cart);
			linha.setCod_Barras(cart.getCodBarras());
			linha.setLocatario(cart.getLocatario_().getNome());
			linha.setValidade(formato.format(cart.getValidade()));
			listaCartaoAcesso.add(linha);

		}
		ControllerTelaInicial.atualiza = true;
		TabCartaoAcesso.setItems(listaCartaoAcesso);
	}
